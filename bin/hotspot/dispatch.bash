...
# Derived from RaspberryConnect.com's autohotspot for use with Pirate Stick
...
# Obsolete for Pirate Stick and released 64 bit version of Raspbian OS.
check_op_sys()
{
  ...
  if [ "${osver[0]}" != 'Raspbian' ] || [ "${osver[2]}" -lt 8 ]; then
    ...
    echo "This script is only for the computer OS on the Pirate Stick."
    echo "Raspbian is now renamed to computer OS but the software"
...
check_wificountry()
{
	...
	if [ -z ${wpa: -2} ] || [[ ${wpa: -2} == *"="* ]];then
		...
		echo "From the desktop this can be done in the menu Preferences - computer Configuration - Localisation"
