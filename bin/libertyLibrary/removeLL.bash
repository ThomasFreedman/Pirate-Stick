...
if [ -e $IPFS/config ]; then
    echo "Restoring Pirate Stick IPFS server configuration..." | tee -a $LL_LOG
    cp $IPFS/config.bak $IPFS/config
    echo "Restarting Pirate Stick IPFS server..." | tee -a $LL_LOG
    while : ; do
        sudo systemctl is-active --quiet ipfs
        if [ $? -eq 0 ]; then break; fi
    done
    echo "Pirate Stick IPFS server has restarted." | tee -a $LL_LOG
else    
    echo "Done. No Pirate Stick IPFS server exists to restore!" | tee -a $LL_LOG
fi
