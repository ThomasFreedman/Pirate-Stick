...
# Content is available through the main Pirate Stick IPFS server node. A
...
# to the main Pirate Stick IPFS server. The PeerID values will be
...
#
# 1. Verify the Pirate Stick IPFS node has been configured (.ipfs/config exists)
...
# 2. Get PeerID of main Pirate Stick IPFS server running on port 4001, 5001
...
#    No popup shows up when this script is run from uDevPython!
    ...
    msg = "Please first run the Pirate Stick Setup Wizard!"
    ...
    if os.path.exists(CFG_LL):
        ...
        # Read the config file for the main Pirate Stick IPFS server and get
        ...
        lg.info(f"Adding Liberty Lib Peer (...{llID[-5:]}) to Pirate Stick config")
        try:
        ...
        lg.info("Restarting Pirate Stick IPFS server to add Liberty Library...")
