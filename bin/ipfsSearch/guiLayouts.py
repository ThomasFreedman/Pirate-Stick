...
class pBoxGUI:
    def __init__(self, ipfs):
        ...
        self.Icon = "/home/ipfs/.config/PirateStick/images/pirateSkullWhite98x128.png"
        ...
        Notify.init("Pirate Stick")                     # Initialize popup notifications
        img = GdkPixbuf.Pixbuf.new_from_file(self.Icon)
        self.Notice = Notify.Notification.new("")
        self.Notice.set_image_from_pixbuf(img)
    ...
    # Terminate a notification immediately, regardless of timer
    def hideNotification(self):
        self.Notice.close()

    # Show a popup notification message with icon
    def showNotification(self, heading, msg, timeout=5000):
        self.Notice.update(heading, msg)
        self.Notice.set_timeout(timeout)
        self.Notice.show()

    # Clean up & exit the notification system
    def exitNotifications(self):
        #self.NotifyObj.uninit()
        Notify.uninit()
    ...
    def progressWindow(self, window, x, y, progress, seconds, max):
        ...
        if window == "open":
            return sg.Window("Progress", 
                             ...
                             location=(x, y), icon=self.Icon, finalize=True)
        elif progress < 0:
            window.close()
        else:
