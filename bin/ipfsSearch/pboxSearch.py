...
#import pyperclip            # Incompatible with PyGObject / gi.repository notifications
import sys
import os
...
def initialize():
    ...
    gui.AppWin = sg.Window('Pirate Stick - IPFS Search', gui.pBoxSearchApp(),
                           location=(200, 200), icon = gui.Icon,
                           finalize=True)   # Set ref main win
    ...
    while True:
        ...
        if event in gui.TopMenuList:
            ...
            # Show some stats about the metadata database in a popup
                ...
                print("fooy")
                cacheLife = sg.popup_get_text(msg, "", pbq.Ipfs.DBcacheTime,
                                    no_titlebar=False, modal=True, size=(20,3),
        ...
        # Open button clicked - launch Chromium web browser with hash (CID) to open
            ...
            if event == '-OPEN-':
                ...
                if fields[0].isdecimal():
                    ...
                    # pyperclip can't work with gi.repository (issues with PyGObject)
                    #pyperclip.copy(hash)
                ...
                sp.Popen(["chromium", "--password-store=basic",
